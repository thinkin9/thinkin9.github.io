---
layout: post
title:  "CS 15 213 LAB1"
date:   2020-08-07 01:38:23 +0900
...
int isTmin(int x) {
    // Tmin: 100...00 (32bits);
    // To detect whether the number is Tmin or not is easily implemented as (x + x) == 000...00
    // The easiest way to detect 0 is !x
    // But, it returns 1 if x = 0;
    // As I mentioned, the easiest way to detect 0 is !x
    // !(x + x) & !(!x)
    return !((x + x) | !x);
}
//2
...
int subOK(int x, int y) {
     ...
     */
    int temp = (1 << 31);  // Detector
    int z = (x + ~y + 1) & temp;  // subtraction
    x = x & temp;
    y = y & temp;
    return !((x ^ y) & (x ^ z));
    /*
...
int isGreater(int x, int y) {
     ...
     */
    return ((~x & y) >> 31) | !((x + ~y + 1) & (1 << 31));  // (Different sign) | (Same sign)
}
//4
/*
 ...
 */
int bitParity(int x) {
    // number of 0's => number of 1's == 32 - number of 0's => odd or even is maintained
    // As learned at Digital Logic course, we can figure out whether x contains an odd or even number of 1's
    // Connect XOR gate to each pair of bits
...
1. Be familiar with Bit-level operations and logical reasoning to implement the given functions
2. Change my perspectives and thought
3. Not fully understand about Floating-Point Representation   
Need to comprehend it and complete last 3 functions related to that